...
class PpgSignalProcessor {
    ...
    companion object {
        ...
        fun calculateSignalQuality(ppgSignal: List<Double>): Double {
            ...
            val normalizedAmplitude = (amplitude / 255.0).coerceIn(0.0, 1.0)
            val normalizedSnr = (snr / 10.0).coerceIn(0.0, 1.0)
            val normalizedVariation = (1.0 - (avgVariation / 50.0).coerceIn(0.0, 1.0))
            
            // Weighted combination of metrics
            return (normalizedAmplitude * 0.4) + (normalizedSnr * 0.4) + (normalizedVariation * 0.2)
        }
        ...
        fun isFingerDetected(redChannel: Double, greenChannel: Double, blueChannel: Double): Boolean {
            ...
            val redThreshold = 160.0  // Minimum red value when finger is present
            val redGreenRatio = 1.3    // Red should be at least this times greater than green
            val redBlueRatio = 1.5     // Red should be at least this times greater than blue
            val maxBrightness = 250.0  // Maximum brightness to avoid saturation
        ...
        private fun removeMotionArtifacts(ppgSignal: List<Double>): List<Double> {
            ...
            val windowSize = 5 // Size of window for artifact detection
            ...
            // Calculate threshold for motion artifact detection
            // Based on statistical properties of the first derivative
            val derivativeMean = firstDerivative.average()
            val derivativeStdDev = sqrt(firstDerivative.map { (it - derivativeMean).pow(2) }.average())
            val motionThreshold = 3.0 * derivativeStdDev // 3 sigma rule
            ...
            for (i in ppgSignal.indices) {
                if (i > 0 && i < ppgSignal.size - 1) {
                    ...
                    if (derivative > motionThreshold) {
                        // Motion artifact detected - replace with interpolated value
                        val startIdx = maxOf(0, i - windowSize)
                        val endIdx = minOf(ppgSignal.size - 1, i + windowSize)
                        
                        // Find valid points before and after the artifact
                        var validBefore = startIdx
                        var validAfter = endIdx
                        
                        for (j in i-1 downTo startIdx) {
                            if (abs(firstDerivative[j]) <= motionThreshold) {
                                validBefore = j
                                break
                            }
                        }
                        ...
                        if (validBefore < validAfter) {
                            ...
                            // Fallback if we can't find valid points
        ...
        fun calculateHr(ppgSignal: List<Double>, samplingRate: Double): Double {
            // Validate input data
            if (ppgSignal.size < 30) {
                return 0.0 // Not enough data for reliable calculation
            }
            
            // Step 1: Check signal quality before processing
            val signalQuality = calculateSignalQuality(ppgSignal)
            if (signalQuality < 0.7) { // Increased quality threshold
            ...
            // Step 3: Calculate signal amplitude and validate
            val signalAmplitude = ppgFiltered.maxOrNull()?.minus(ppgFiltered.minOrNull() ?: 0.0) ?: 0.0
            if (signalAmplitude < 5.0) { // Minimum amplitude threshold
                return 0.0 // Signal too weak
            }
            
            // Step 4: Detect peaks with stricter criteria
            ...
            for (i in windowSize until ppgFiltered.size - windowSize) {
                ...
                if (ppgFiltered[i] == localMax && 
                    ...
                    if (ppgFiltered[i] > ppgFiltered.average() + signalAmplitude * 0.3) {
                        peaks.add(i)
                    }
                }
            }
            ...
            for (i in 1 until peaks.size) {
                ...
                // Only accept physiologically plausible intervals (45-120 BPM)
                val instantBpm = 60.0 / interval
                if (instantBpm in 45.0..120.0) {
            ...
            // Step 7: Convert IBI to Heart Rate (BPM = 60 / IBI in seconds)
            val heartRate = if (medianInterval > 0) 60.0 / medianInterval else 0.0
            
            // Step 8: Validate the calculated heart rate against physiological limits
            return when {
                heartRate < 40 || heartRate > 200 -> 0.0 // Outside physiological range
                else -> heartRate
            }
        }
